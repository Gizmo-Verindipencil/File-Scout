...
{
    /// <summary>
    /// 行文字数の標準偏差の調査手段のテストを提供します。
    /// </summary>
    [TestClass]
    public class RowTextLengthStandardDeviationScoutingMethodUnitTest
    {
        /// <summary>
        /// テスト用ディレクトリのパスを取得します。
        /// </summary>
        private string DirectoryPath
        {
            get
            {
                ...
                var dir = Path.Combine(root, nameof(RowTextLengthStandardDeviationScoutingMethodUnitTest));
        ...
        public void Test_Do_ReturnsZero_WhenFileIsBinary()
        {
            ...
            // テスト対象の処理を実行
            var method = new RowTextLengthStandardDeviationScoutingMethod();
        ...
        /// <summary>
        /// 行文字数の標準偏差(小数部なし)の調査結果を検証します。
        /// </summary>
        [TestMethod]
        public void Test_Do_ReturnsStandardDeviationWithoutDecimalPoint()
        {
            // テスト用のファイルを用意
            var fileName = MethodBase.GetCurrentMethod().Name;
            var filePath = Path.Combine(this.DirectoryPath, fileName);
        ...
        /// <summary>
        /// 行文字数の標準偏差(小数部あり)の調査結果を検証します。
        /// </summary>
        [TestMethod]
        public void Test_Do_ReturnsStandardDeviationWithDecimalPoint()
        {
            // テスト用のファイルを用意
            var fileName = MethodBase.GetCurrentMethod().Name;
            var filePath = Path.Combine(this.DirectoryPath, fileName);
            ...
            // テスト結果を検証
            Assert.AreEqual("0.816496580927726", actual);
        }
    }
}
